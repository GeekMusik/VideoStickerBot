...
async def main(_, msg: Message):
    status = await msg.reply("Melakukan yang terbaik yang saya lakukan...")
...
async def process(msg: Message, stark: Helpers, status: Message):
    ...
    if os.path.exists(stark.output_file):
        ...
        await status.edit('Mendapatkan paket Anda...')
        ...
        await status.delete()
        if sticker:
            button = [[InlineKeyboardButton('Mendapatkan WEBM File', callback_data='current_webm')]]
...
async def existing_sticker_func(_, msg: Message):
    if msg.sticker.is_video:
        ...
        if data['kang_mode']:
            status = await msg.reply("Melakukan yang terbaik yang saya lakukan [Kanging]... ")
