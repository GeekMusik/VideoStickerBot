...
async def change_default_emojis_cb_func(bot: Stark, query: CallbackQuery):
    user_id = query.from_user.id
    await query.answer()
    emojis_msg = await bot.ask(user_id, 'Tolong kirimi saya emoji', filters=filters.text & filters.incoming)
    emojis = await Helpers.extract_emojis(emojis_msg)
    if not emojis:
        await emojis_msg.reply('Tidak ditemukan emoji yang valid. Proses Keluar', quote=True)
        return
...
async def remove_default_emojis_cb_func(_, query: CallbackQuery):
    ...
    if data:
        ...
        await query.answer("Emoji default tidak disetel", show_alert=True)
...
async def change_bool(key, query):
    ...
    if text == query.message.text:
        await query.answer('Sebuah pesan lama. Menghapus...')
        await query.message.delete()
        return
    if not text:
        await query.answer('Terjadi kesalahan. Coba kedua kalinya, Jika masih ada, tanyakan di obrolan dukungan')
        return
    await query.edit_message_text(text, reply_markup=markup)
    await query.answer()
