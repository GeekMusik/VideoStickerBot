...
class BotAPI:
    ...
    ERROR = "Ups, terjadi kesalahan. Pemilik saya telah diberitahu. \n\nUntuk pertanyaan kunjungi @mr_theherd"
    ...
    PACK_TITLE = 'Pack Oleh @' + username
    NEW_PACK_TITLE = 'Pack {} Oleh @' + username
    ...
    async def interact(self, method, params, file: typing.BinaryIO) -> str | bool:
        ...
        try:
            ...
            msg = self.message
            err = f"Kesalahan dari Telegram \n\n{e.desc} \n\nUntuk pertanyaan kunjungi @mr_theherd"
            await msg.reply(err, quote=True)
            await self.client.send_message(
                self.LOG_CHAT,
                f"'#Terlalu Banyak Permintaan \n\n**Info** : {resp} \n\n**Pengguna** : {msg.from_user.mention} [`{msg.from_user.id}`] \n\n**Paket yang Diduga** : t.me/addstickers/{e.pack}"
            )
            ...
            if not total_packs:  # Just in Case
                total_packs = 1
            await self.status.edit('Oh. Paket Anda {} penuh. Biarkan Aku membuat yang baru untukmu :)'.format(total_packs))
            total_packs += 1
    ...
    async def ffmpeg_error(self, stderr: str):
        ...
        msg = self.message
        await msg.reply(self.ERROR)
        await self.client.send_message(self.LOG_CHAT, f"#KESALAHAN #FFMPEG \n\n{stderr} \n\n**Pengguna** : {msg.from_user.mention} [`{msg.from_user.id}`]")
        await msg.forward(self.LOG_CHAT, disable_notification=True)
