...
class Helpers(BotAPI):
    ...
    async def user_settings(self) -> tuple[bool, bool] | tuple[str, InlineKeyboardMarkup]:
        ...
        if ask_emojis_db:
            ask_emojis += tick
            text += f"**Minta Emoji** : True"
        else:
            ask_emojis += cross
            text += f"**Minta Emoji** : False"
    ...
    async def ask_for_emojis(self):
        emojis_msg = await self.client.ask(
            self.user_id,
            "Kirimkan saya emoji untuk ditambahkan ke stiker ini.",
            filters=filters.text & filters.incoming
        )
        status = await emojis_msg.reply('Memeriksa Masukan...')
        emojis = await self.extract_emojis(emojis_msg)
        if not emojis:
            await status.edit('Emoji tidak valid. Proses Dibatalkan.')
            return None
    ...
    async def correct_the_size(self):
        ...
        kib = info // 1024
        if kib > 256:
            await self.status.edit('Mengubah ukuran Video agar sesuai dengan Batas Telegram...')
